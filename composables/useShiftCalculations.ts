...
export const useShiftCalculations = (
  ...
  const detectedWorkflow = computed(() => {
    const start =
      form.value.beginning_box !== undefined ? form.value.beginning_box : 4000;
    const end =
      form.value.ending_box !== undefined ? form.value.ending_box : 4000;
  ...
  const calculate = (
    triggerSource: "bingo" | "deposit" | "pulltabs" | "box" | "cash_count",
  ) => {
    ...
    const bingoInput = form.value.bingo_total_input || 0;
