...
async function main() {
  ...
  const box2 = frameWith([0,1,2,3,4, 5,9, 10,14, 15,19, 20,21,22,23,24]); // outer box

  // 5. Hard Way (Static)
  const hardWayFrame = frameWith([5,6,7,8,9, 10,14, 15,19, 20,21,22,23,24]); // not really hard way but illustrative
  ...
  // Fetch IDs
  const eveningProg = await prisma.bingoProgram.findUnique({ where: { slug: "evening-main-program" } });
  const sundayProg = await prisma.bingoProgram.findUnique({ where: { slug: "sunday-premier-program" } });

  const pCoverall = await prisma.bingoPattern.findUnique({ where: { slug: "coverall" } });
  const pLetterX = await prisma.bingoPattern.findUnique({ where: { slug: "letter-x" } });
  const pCorners = await prisma.bingoPattern.findUnique({ where: { slug: "four-corners" } });
  const pPulse = await prisma.bingoPattern.findUnique({ where: { slug: "pulsing-box" } });
  const pHard = await prisma.bingoPattern.findUnique({ where: { slug: "hard-way" } });

  const eveningGames = [
    { sort_order: 1, title: "Warm Up", paperColor: "#FFC0CB", pattern_id: pCorners.id, notes: "Early bird special" }, // Pink
    { sort_order: 2, title: "Double Action", paperColor: "#ADD8E6", pattern_id: pLetterX.id }, // LightBlue
    { sort_order: 3, title: "Jackpot Special", paperColor: "#90EE90", pattern_id: pCoverall.id, notes: "$1000 Guaranteed" }, // LightGreen
    { sort_order: 4, title: "Intermission Fun", paperColor: "#FFFFE0", pattern_id: pPulse.id }, // LightYellow
    { sort_order: 5, title: "Last Chance", paperColor: "#D3D3D3", pattern_id: pHard.id }, // LightGray
  ];

  const sundayGames = [
    { sort_order: 1, title: "Super Starter", paperColor: "#FFA07A", pattern_id: pCorners.id }, // LightSalmon
    { sort_order: 2, title: "Big X", paperColor: "#E0FFFF", pattern_id: pLetterX.id }, // LightCyan
    { sort_order: 3, title: "Mega Coverall", paperColor: "#FFD700", pattern_id: pCoverall.id, notes: "Progressive Jackpot" }, // Gold
    { sort_order: 4, title: "Crazy Box", paperColor: "#FF69B4", pattern_id: pPulse.id }, // HotPink
  ];
