...
export const pricingCompiler = {
  ...
  getSpecialsByDay(content: PricingSchemaV2): Record<string, PricingPromotion[]> {
      ...
      for (let i = 0; i < 7; i++) {
        ...
        const recurring = content.promotions.filter(p => {
            if (!p.isActive) return false;
  ...
  compileForPublic(
    ...
    date: Date = new Date(),
  ): PricingConfig & { promotions?: PricingPromotion[]; todaySpecials?: PricingPromotion[]; weekSpecialsByDay?: Record<string, PricingPromotion[]>; meta?: any } {
