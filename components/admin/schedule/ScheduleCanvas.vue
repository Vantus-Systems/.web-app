<template>
  ...
  >
    <!-- Sticky Header -->
    <div
      class="grid grid-cols-7 border-b border-divider bg-surface z-20 sticky top-0 shadow-sm"
    >
      <div
    ...
    <div
      ...
      class="flex-1 overflow-y-auto overflow-x-hidden bg-base custom-scrollbar"
    >
      <!-- Top Loader Trigger -->
      ...
      <div
        v-if="weeks.length === 0"
        class="flex items-center justify-center h-full text-secondary"
      >
        No dates in range
      </div>
      ...
      <div v-else class="relative" v-bind="wrapperProps">
        ...
        >
          <div
            ...
            :class="{
              'bg-accent-primary/5 ring-inset ring-2 ring-accent-primary/20':
          ...
          >
            ...
            >
              <span
                class="text-3xl font-black text-primary/5 leading-none select-none"
              >
...
const startPaint = (date: string, event: MouseEvent) => {
  ...
  if (props.activeToolProgramSlug) {
    ...
    emit("paint-range", {
      dates: [date],
      programSlug: props.activeToolProgramSlug,
    });
  } else {
    // Selection mode
    if (event.shiftKey) {
      ...
      const alreadySelected =
        props.selectedDates.length === 1 && props.selectedDates.includes(date);
      if (alreadySelected) {
...
const getEffectiveAssignment = (date: string) => {
  return resolveEffectiveAssignment(date, props.slots, props.overrides);
};
...
const getGhostProgram = (date: string): BingoProgramExtended | undefined => {
  if (
    isPainting.value &&
    props.activeToolProgramSlug &&
    paintStartKey.value &&
    activeHoverDate.value
  ) {
    const range = getRange(paintStartKey.value, activeHoverDate.value);
