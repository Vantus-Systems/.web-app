...
const handleDrop = (event: DragEvent, dayIndex: number, timeVal: number) => {
  const programSlug = draggingProgram.value || event.dataTransfer?.getData('text/plain');
...
const getProgram = (slug: string) => programs.value.find(p => p.slug === slug);
...
const publish = async () => {
  if (!confirm("Are you sure you want to publish this schedule? This will update the live public schedule.")) return;
  ...
  <div class="flex h-full bg-base overflow-hidden">
    ...
    <div class="w-64 bg-surface border-r border-divider flex flex-col shrink-0">
      <div class="p-4 border-b border-divider">
        <h3 class="font-bold text-sm text-secondary uppercase tracking-wider mb-2">Programs</h3>
        <div class="relative">
          <Search class="absolute left-2 top-1/2 -translate-y-1/2 w-4 h-4 text-muted" />
          <input
            v-model="searchQuery"
            type="text"
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-xs text-secondary truncate">{{ program.slug }}</div>
            </div>
          </div>
        </div>
        
        <div v-if="programs.length === 0" class="p-4 text-center text-sm text-secondary">
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-base">
      <!-- Toolbar -->
      <div class="h-14 border-b border-divider flex items-center justify-between px-4 bg-surface shrink-0">
        <div class="flex items-center gap-4">
          <h2 class="font-bold text-primary">Weekly Schedule</h2>
          <div v-if="scheduleStore.dirty" class="flex items-center gap-2 text-amber-600 text-xs font-bold uppercase">
        ...
        <div class="flex items-center gap-2">
          ...
          >
            <Save class="w-4 h-4" :class="{ 'animate-pulse': scheduleStore.saving }" />
            Save
          </button>
          ...
          >
            <UploadCloud class="w-4 h-4" :class="{ 'animate-pulse': scheduleStore.publishing }" />
            Publish
          </button>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-auto relative">
        <div class="min-w-[800px] flex">
          <!-- Time Labels -->
          <div class="w-16 flex-shrink-0 bg-surface border-r border-divider z-10 sticky left-0">
            <div class="h-10 border-b border-divider"></div> <!-- Header spacer -->
            <div 
          ...
          <div class="flex-1 flex">
            ...
            >
              <!-- Day Header -->
              <div class="h-10 border-b border-divider bg-surface sticky top-0 z-10 flex items-center justify-center font-medium text-sm text-secondary">
                {{ day }}
              </div>
              ...
              <div class="relative">
                ...
                <div
                  ...
                  :class="selectedSlotId === slot.id ? 'bg-accent-primary text-white border-accent-primary z-20 ring-2 ring-accent-primary/30' : 'bg-surface text-primary border-input hover:border-primary z-10'"
                  :style="getSlotStyle(slot)"
                  @click.stop="selectedSlotId = slot.id"
                >
                  <div class="font-bold truncate">{{ getProgram(slot.program_slug)?.name || slot.program_slug }}</div>
                  <div class="truncate opacity-80">{{ slot.start_time }} ({{ slot.duration_minutes }}m)</div>
                </div>
              </div>
            </div>
          </div>
    ...
    <div class="w-80 bg-surface border-l border-divider flex flex-col shrink-0" v-if="selectedSlot">
      <div class="p-4 border-b border-divider flex items-center justify-between">
        <h3 class="font-bold text-sm text-secondary uppercase tracking-wider">Slot Details</h3>
        <button 
      ...
      <div class="p-4 space-y-4 overflow-y-auto flex-1">
        <div class="space-y-1">
          ...
          <select 
            ...
            @change="updateSelectedSlot({ program_slug: ($event.target as HTMLSelectElement).value })"
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="space-y-1">
            ...
            <input 
              ...
              @change="updateSelectedSlot({ start_time: ($event.target as HTMLInputElement).value })"
              class="w-full bg-base border border-input rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
            />
          </div>
          <div class="space-y-1">
            <label class="text-xs font-medium text-secondary">Duration (min)</label>
            <input 
              type="number" 
              :value="selectedSlot.duration_minutes"
              @change="updateSelectedSlot({ duration_minutes: parseInt(($event.target as HTMLInputElement).value) })"
              step="30"
              min="30"
              class="w-full bg-base border border-input rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
            />
        ...
        <div class="space-y-1">
           ...
           <select
             ...
             @change="updateSelectedSlot({ day_of_week: parseInt(($event.target as HTMLSelectElement).value) })"
             class="w-full bg-base border border-input rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-primary"
           >
    ...
    <div v-else class="w-80 bg-surface border-l border-divider flex items-center justify-center text-secondary text-sm p-8 text-center">
