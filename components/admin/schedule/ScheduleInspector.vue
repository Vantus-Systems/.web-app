<template>
  <div
    class="h-full flex flex-col bg-surface border-l border-divider overflow-y-auto"
  >
    <!-- Context Header -->
    ...
    <div v-if="mode === 'summary'" class="p-4 space-y-6">
      ...
      <div class="pt-4 border-t border-divider space-y-3">
        ...
        <p class="text-[10px] text-tertiary">
          Apply a profile to specific weekdays across the entire range.
        </p>
      ...
      <div class="pt-4 border-t border-divider">
        ...
        <p class="text-xs text-tertiary leading-relaxed">
          {{
            stats.conflicts > 0
              ? "Review conflicts marked in red on the calendar."
              : "All checks passed."
          }}
        </p>
      </div>
    ...
    <div v-else-if="mode === 'single'" class="p-4 space-y-6">
      ...
      <div v-if="selectedAssignment.effectiveProfileId" class="space-y-4">
        <div class="p-3 bg-base border border-divider rounded-lg">
          <div class="text-xs font-bold text-tertiary uppercase mb-1">
            Assigned Profile
          </div>
          <div class="font-bold text-accent-primary">
            {{ selectedProfile?.name || "Unknown Profile" }}
          </div>
          <div
            v-if="selectedProfile?.doors_open_time"
        ...
        <!-- Overrides Display -->
        <div
          v-if="selectedAssignment.overrideReasons.length"
          class="p-3 bg-accent-info/5 border border-accent-info/20 rounded-lg"
        >
          <div class="text-xs font-bold text-accent-info uppercase mb-1">
            Active Overrides
          </div>
          <ul class="list-disc list-inside text-xs text-secondary">
            <li
              v-for="(reason, idx) in selectedAssignment.overrideReasons"
              :key="idx"
            >
              {{ reason }}
            </li>
        ...
        <div>
          <div class="text-xs font-bold text-tertiary uppercase mb-2">
            Timeline Preview
          </div>
      ...
      <div
        v-else-if="selectedAssignment.status === 'closed'"
      ...
      <div v-else class="text-center py-8 text-tertiary text-sm">
        ...
        <span class="text-xs opacity-70"
          >Select a profile from the library to assign.</span
        >
      </div>

      <!-- Overrides Editor (Quick Actions) -->
      <div class="pt-4 border-t border-divider space-y-2">
        <label class="text-xs font-bold text-secondary uppercase"
          >Overrides</label
        >
        <div class="grid grid-cols-2 gap-2">
          <div class="col-span-1">
            <label
              class="text-[9px] text-tertiary uppercase font-bold block mb-1"
              >Doors Open</label
            >
            <div class="flex gap-1">
              <input
                v-model="overrideDoorsTime"
                type="time"
                class="flex-1 text-xs bg-base border border-divider rounded px-1 py-1"
              />
              <button
                class="px-2 border border-divider rounded text-xs hover:bg-base"
                @click="emitOverride('DOORS_OPEN')"
              >
                Set
              </button>
            </div>
          ...
          <div class="col-span-1">
            ...
            <div class="flex gap-1">
              <input
                v-model="overrideCloseTime"
                type="time"
                class="flex-1 text-xs bg-base border border-divider rounded px-1 py-1"
              />
              <button
                class="px-2 border border-divider rounded text-xs hover:bg-base"
                @click="emitOverride('CLOSE_EARLY')"
              >
                Set
              </button>
            </div>
          ...
          <button
            class="p-2 border border-divider rounded text-xs hover:bg-base"
            @click="emitOverride('CLOSED')"
          >
            Close Day
          </button>
          <button
            class="p-2 border border-divider rounded text-xs hover:bg-base"
            @click="emitOverride('LOCKED')"
          >
            Lock Day
          </button>
        </div>
    ...
    <div v-else-if="mode === 'multi'" class="p-4 space-y-6">
      <div
        class="bg-accent-primary/10 border border-accent-primary/20 rounded-lg p-4 text-center"
      >
        <div class="text-3xl font-black text-accent-primary">
          {{ selectedDates.length }}
        </div>
        <div
          class="text-xs font-bold text-accent-primary uppercase tracking-wide"
        >
          Days Selected
        </div>
      </div>
      ...
      <div class="space-y-2">
        <label class="block text-xs font-bold text-secondary uppercase"
          >Bulk Actions</label
        >
...
const selectedAssignment = computed(() => {
  if (mode.value !== "single")
    return { status: "open", effectiveProfileId: null, overrideReasons: [] };
...
const selectedProfile = computed(() => {
  ...
  return props.profiles.find(
    (p) => p.id === selectedAssignment.value.effectiveProfileId,
  );
});
