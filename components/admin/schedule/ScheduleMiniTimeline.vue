<template>
  <div
    class="p-4 bg-surface rounded-lg shadow-xl border border-divider w-[500px]"
  >
    <div
      class="flex items-center justify-between mb-4 border-b border-divider pb-2"
    >
      <div>
      ...
      <button class="text-tertiary hover:text-primary" @click="$emit('close')">
        <svg
    ...
    <!-- Timeline Viz -->
    <div
      class="relative h-24 bg-base rounded-md border border-divider overflow-hidden mb-4"
    >
      <!-- Hours Markers -->
      <div
        class="absolute top-0 bottom-0 w-full flex justify-between px-2 text-[9px] text-tertiary select-none pointer-events-none z-0"
      >
        <span
          v-for="h in 24"
          :key="h"
      ...
      <template v-if="profile && timeline">
        ...
        >
          <!-- Tooltip -->
          <div
            class="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 bg-surface-dark text-white text-[10px] px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none"
          >
            {{ segStyles.segment.label }} ({{ segStyles.segment.time_start }}-{{
              segStyles.segment.time_end
            }})
          </div>
        </div>
      </template>
      ...
      <template v-if="profile && timeline">
        ...
        >
          <div
            class="absolute -top-1 -left-1 w-2.5 h-2.5 bg-accent-warning rounded-full border-2 border-white"
            :title="evt.label"
          />
        </div>
      </template>
...
const resolvedSegments = computed(() => {
  ...
  const segs = props.timeline.flowSegments.filter((s) =>
    props.profile!.segment_ids.includes(s.id),
  );
...
const resolvedEvents = computed(() => {
  if (!props.profile || !props.timeline) return [];
  return props.timeline.overlayEvents.filter((e) =>
    props.profile!.overlay_event_ids.includes(e.id),
  );
});
