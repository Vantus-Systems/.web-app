<template>
  <div 
    class="fixed z-50 min-w-[180px] bg-white rounded-lg shadow-xl border border-slate-200 py-1 overflow-hidden"
    :style="{ top: y + 'px', left: x + 'px' }"
  >
    <div class="px-3 py-2 border-b border-slate-100 bg-slate-50">
      <div class="text-xs font-bold text-slate-900">{{ dateLabel }}</div>
    </div>
    
    <button 
      class="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-primary-600 flex items-center gap-2"
      @click="$emit('paste')"
    >
    ...
    <button 
      class="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-amber-600 flex items-center gap-2"
      @click="$emit('lock')"
    >
    ...
    <button 
      class="w-full text-left px-3 py-2 text-sm text-slate-600 hover:bg-slate-50 hover:text-indigo-600 flex items-center gap-2"
      @click="$emit('holiday')"
    >
