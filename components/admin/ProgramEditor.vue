...
const getPattern = (slug: string) => patterns.value.find(p => p.slug === slug);
</script>
...
<template>
  <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        >
          <div class="flex justify-between items-start mb-4">
             ...
             <button @click="deleteProgram(p.slug)" class="text-red-400 hover:text-red-600">
               <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
             </button>
          </div>

          <div class="space-y-2 mb-6 max-h-48 overflow-y-auto">
             <div v-for="g in p.games" :key="g.id" class="flex items-center gap-2 text-sm border-b border-slate-50 py-1">
                <div class="w-3 h-3 rounded-full border border-black/10" :style="{backgroundColor: g.paperColor}"></div>
    ...
    <!-- Edit View -->
    <div v-else class="max-w-4xl mx-auto bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
       <h2 class="text-2xl font-bold mb-6">{{ form.slug ? 'Edit Program' : 'Create Program' }}</h2>

       <div class="grid grid-cols-2 gap-6 mb-8">
         <div class="col-span-2 md:col-span-1">
            <label class="block text-sm font-bold text-slate-700 mb-1">Name</label>
            <input v-model="form.name" type="text" class="w-full border-slate-300 rounded-lg p-2" />
         </div>
         <div class="col-span-2 md:col-span-1">
            <label class="block text-sm font-bold text-slate-700 mb-1">Slug</label>
            <input v-model="form.slug" type="text" class="w-full border-slate-300 rounded-lg p-2" :disabled="!editingProgram.isNew" />
         </div>
         <div class="col-span-2">
            <label class="block text-sm font-bold text-slate-700 mb-1">Description</label>
            <input v-model="form.description" type="text" class="w-full border-slate-300 rounded-lg p-2" />
         </div>
       </div>
       ...
       <div class="mb-8">
          <div class="flex justify-between items-center mb-4">
             ...
             <button @click="addGame" class="bg-primary-600 text-white px-3 py-1.5 rounded-lg text-sm font-bold hover:bg-primary-700">+ Add Game</button>
          </div>
          ...
          <div class="space-y-3">
             ...
             >
                <div class="flex flex-col gap-1">
                   <button @click="moveGame(idx, -1)" :disabled="idx === 0" class="text-slate-400 hover:text-slate-700 disabled:opacity-20">▲</button>
                   <span class="text-center font-bold text-xs text-slate-500">{{ idx + 1 }}</span>
                   <button @click="moveGame(idx, 1)" :disabled="idx === form.games.length - 1" class="text-slate-400 hover:text-slate-700 disabled:opacity-20">▼</button>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 flex-1 w-full">
                   <div class="col-span-1">
                      ...
                      <input v-model="game.title" type="text" class="w-full border-slate-300 rounded p-1.5 text-sm" />
                   </div>

                   <div class="col-span-1">
                      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Paper Color</label>
                      <div class="flex gap-2">
                         <input v-model="game.paperColor" type="color" class="h-8 w-10 p-0 border-0 rounded" />
                         <input v-model="game.paperColor" type="text" class="w-full border-slate-300 rounded p-1.5 text-sm" placeholder="#RRGGBB" />
                      </div>
                   </div>

                   <div class="col-span-1">
                      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Pattern</label>
                      <select v-model="game.patternSlug" class="w-full border-slate-300 rounded p-1.5 text-sm">
                         <option v-for="p in patterns" :key="p.slug" :value="p.slug">{{ p.name }}</option>
                      </select>
                   </div>

                   <div class="col-span-1">
                      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Notes</label>
                      <input v-model="game.notes" type="text" class="w-full border-slate-300 rounded p-1.5 text-sm" placeholder="Optional..." />
                   </div>
                </div>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <button @click="removeGame(idx)" class="text-red-400 hover:text-red-600 text-xs font-bold">Remove</button>
                </div>
             </div>

             <div v-if="form.games.length === 0" class="text-center py-8 text-slate-400 italic">
                No games added yet.
             </div>
          </div>
       </div>
       ...
       <div class="flex justify-end gap-4">
          <button @click="cancelEdit" class="px-6 py-2 text-slate-500 font-bold hover:text-slate-700">Cancel</button>
          <button
            @click="saveProgram"
            :disabled="isSaving"
