  ...
  <div class="h-full flex flex-col">
          ...
          <div class="flex items-center justify-between">
            <div>
              <div class="flex items-center gap-2">
                ...
                <span v-if="template.id === modelValue.defaultTemplateId" class="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-[10px] font-bold uppercase rounded-full">Default</span>
              </div>
              <p class="text-sm text-secondary">{{ template.description || 'No description' }}</p>
            </div>
    ...
    <div v-else class="h-full flex flex-col">
      <div class="flex items-center gap-4 mb-6 pb-4 border-b border-divider">
        <button @click="editingTemplate = null" class="text-sm font-medium text-secondary hover:text-primary">
...
watch(() => editingTemplate.value?.name, () => {
  saveTemplateChanges();
});
...
function deleteTemplate(id: string) {
  ...
  const isUsedInRotation = Object.values(props.modelValue.weeklyRotation).includes(id);
  const isUsedInOverrides = props.modelValue.dateOverrides.some(o => o.templateId === id);
  
  if (isUsedInRotation || isUsedInOverrides) {
    alert('Cannot delete template because it is used in Weekly Rotation or Date Overrides. Please remove usages first.');
    return;
  }
