<template>
  <div class="h-full flex flex-col bg-slate-50 relative overflow-hidden">
    ...
    <div class="h-10 bg-white border-b border-slate-200 flex sticky top-0 z-10 shrink-0">
      <div class="w-20 border-r border-slate-200 shrink-0 bg-slate-50"></div> <!-- Sidebar spacer -->
    ...
    <div class="flex-1 overflow-y-auto overflow-x-hidden relative">
        ...
        <div class="flex h-full">
            <!-- Labels Column -->
            <div class="w-20 border-r border-slate-200 bg-white shrink-0 flex flex-col pt-4">
                <div class="h-12 flex items-center justify-end px-2 text-[10px] font-bold uppercase text-slate-400">Flow</div>
                <div class="h-12 flex items-center justify-end px-2 text-[10px] font-bold uppercase text-slate-400 mt-2">Events</div>
            </div>
            ...
            <div class="flex-1 relative pt-4" ref="tracksArea">
                ...
                <div class="h-12 relative w-full mb-2">
                    <div class="absolute inset-0 bg-slate-100/50 mx-2 rounded-lg border border-slate-200/50"></div>
                    <div v-for="segment in flowSegments" :key="segment.id"
                        ...
                        :class="selectedId === segment.id ? '!bg-blue-300 !border-blue-500 ring-2 ring-blue-500/30' : ''"
                        :style="getSegmentStyle(segment)"
                        @click="$emit('select', { type: 'flowSegment', id: segment.id })"
                    >
                        <span class="text-[10px] font-bold text-blue-700 truncate">{{ segment.name }}</span>
                    </div>
                </div>
                ...
                <div class="h-12 relative w-full mt-2">
                     <div class="absolute inset-0 bg-slate-100/50 mx-2 rounded-lg border border-slate-200/50"></div>
                     <div v-for="event in overlayEvents" :key="event.id"
                        ...
                        :class="selectedId === event.id ? '!bg-purple-300 !border-purple-500 ring-2 ring-purple-500/30' : ''"
                        :style="getSegmentStyle(event)"
                        @click="$emit('select', { type: 'overlayEvent', id: event.id })"
                    >
                        <span class="text-[10px] font-bold text-purple-700 truncate">{{ event.name }}</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Legend / Tools -->
    <div class="h-12 bg-white border-t border-slate-200 flex items-center px-4 justify-between shrink-0">
        <div class="flex items-center gap-4 text-xs text-slate-500">
            <div class="flex items-center gap-1"><div class="w-3 h-3 bg-blue-100 border border-blue-200 rounded"></div> Flow Segments</div>
            <div class="flex items-center gap-1"><div class="w-3 h-3 bg-purple-100 border border-purple-200 rounded"></div> Overlays</div>
        </div>
        <div class="flex gap-2">
            <button class="px-3 py-1 text-xs font-bold uppercase bg-blue-50 text-blue-700 rounded hover:bg-blue-100" @click="$emit('addFlowSegment')">
                + Segment
            </button>
            <button class="px-3 py-1 text-xs font-bold uppercase bg-purple-50 text-purple-700 rounded hover:bg-purple-100" @click="$emit('addOverlayEvent')">
                + Event
            </button>
        </div>
    </div>
