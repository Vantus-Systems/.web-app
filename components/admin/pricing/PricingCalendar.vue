  ...
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button class="p-1 hover:bg-base rounded" @click="prevMonth">
          &lt;
        </button>
        <span class="font-bold">{{ currentMonthName }} {{ currentYear }}</span>
        <button class="p-1 hover:bg-base rounded" @click="nextMonth">
          &gt;
        </button>
      </div>
    </div>
    ...
    <div
      class="grid grid-cols-7 gap-1 text-center text-xs font-bold text-secondary uppercase tracking-wider mb-2"
    >
      <div
    ...
    <div class="mt-8 p-6 bg-surface border border-divider rounded-xl">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
        ...
        >
          <label
            class="block text-xs font-bold text-secondary uppercase mb-1"
            >{{ day }}</label
          >
          <select
      ...
      <div class="bg-surface p-6 rounded-xl shadow-xl max-w-md w-full">
        ...
        <div class="space-y-4">
          <div>
            ...
            >
            <select
              v-model="overrideForm.templateId"
              class="w-full rounded-md border-divider bg-base p-2"
            >
              <option value="">
                No Override (Use {{ getEffectiveTemplateName(selectedDate) }})
              </option>
              <option
                v-for="t in modelValue.templates"
                :key="t.id"
                :value="t.id"
              >
                {{ t.name }}
        ...
        <div class="mt-6 flex justify-end gap-3">
          <button
            class="px-4 py-2 text-sm font-medium text-secondary hover:text-primary"
            @click="selectedDate = null"
          >
            Cancel
          </button>
          <button
            class="px-4 py-2 bg-accent-primary text-white rounded-md text-sm font-bold"
            @click="saveOverride"
          >
            Save
          </button>
...
const defaultTemplateName = computed(() => {
  const t = props.modelValue.templates.find(
    (t) => t.id === props.modelValue.defaultTemplateId,
  );
...
function createDayObject(date: Date, isCurrentMonth: boolean) {
  ...
  const override = props.modelValue.dateOverrides.find(
    (o) => o.date === dateStr,
  );
...
function saveOverride() {
  ...
  const newOverrides = [
    ...props.modelValue.dateOverrides.filter((o) => o.date !== dateStr),
  ];
