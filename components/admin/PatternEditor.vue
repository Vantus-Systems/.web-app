...
const startEdit = (p?: any) => {
  if (p) {
    ...
    form.value = {
      ...
      isAnimated: false,
      definition: {
        frames: [Array(25).fill(0).map((_, i) => i === 12 ? 1 : 0)], // Default with free space? Or empty?
        interval: 500,
      }
    };
  }
...
const deletePattern = async (slug: string) => {
  if (!confirm("Are you sure? This may break programs using this pattern.")) return;
  try {
...
<template>
  <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          <div class="flex justify-between items-start mb-4">
             ...
             <button @click="deletePattern(p.slug)" class="text-red-400 hover:text-red-600">
               <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"/><path d="M19 6v14c0 1-1 2-2 2H7c-1 0-2-1-2-2V6"/><path d="M8 6V4c0-1 1-2 2-2h4c1 0 2 1 2 2v2"/></svg>
             </button>
          </div>

          <div class="flex justify-center mb-4 cursor-pointer" @click="startEdit(p)">
            <BingoPatternGrid
              :name="p.name"
          ...
          <div class="flex items-center justify-between text-xs text-slate-500">
             <span>{{ p.slug }}</span>
             <span v-if="p.isAnimated" class="bg-purple-100 text-purple-700 px-2 py-0.5 rounded">Animated</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Edit View -->
    <div v-else class="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-xl border border-slate-100">
       <h2 class="text-2xl font-bold mb-6">{{ form.slug ? 'Edit Pattern' : 'Create Pattern' }}</h2>
       ...
       <div class="space-y-4 mb-8">
         ...
         <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input v-model="form.slug" type="text" class="w-full border-slate-300 rounded-lg p-2" :disabled="!editingPattern.isNew" />
            </div>
            <div class="flex items-center mt-6">
                <input v-model="form.isAnimated" type="checkbox" id="anim" class="rounded text-gold focus:ring-gold" />
                <label for="anim" class="ml-2 text-sm font-bold text-slate-700">Animated Pattern</label>
            </div>
         </div>
         <div>
            <label class="block text-sm font-bold text-slate-700 mb-1">Description</label>
            <textarea v-model="form.description" class="w-full border-slate-300 rounded-lg p-2" rows="2"></textarea>
         </div>

         <div v-if="form.isAnimated">
            <label class="block text-sm font-bold text-slate-700 mb-1">Interval (ms)</label>
            <input v-model="form.definition.interval" type="number" step="50" min="50" max="5000" class="w-full border-slate-300 rounded-lg p-2" />
         </div>
       </div>
       ...
       <div class="mb-8">
          <div class="flex items-center justify-between mb-2">
             <label class="block text-sm font-bold text-slate-700">Frames ({{ form.definition.frames.length }})</label>
             <div class="space-x-2">
                <button v-if="form.isAnimated" @click="addFrame" class="text-xs bg-slate-100 px-2 py-1 rounded hover:bg-slate-200">+ Add Frame</button>
             </div>
          </div>

          <div v-if="form.definition.frames.length > 1" class="flex gap-2 overflow-x-auto mb-4 pb-2">
             <button
          ...
          <div class="flex flex-col items-center">
             <div class="grid grid-cols-5 gap-1 bg-slate-200 p-1 rounded mb-4 w-64 h-64">
                <div
                  ...
                  :style="val === 1 || idx === 12 ? { backgroundColor: idx === 12 ? undefined : '#eab308' } : {}"
                >
                   <span v-if="idx === 12" class="text-xs font-bold text-slate-400">FREE</span>
                </div>
             </div>
