  ...
  <div class="h-full bg-white border-l border-slate-200 p-4 space-y-4">
    ...
    <div v-if="selectedItem" class="space-y-4">
      <div v-if="selected?.type === 'rateCard'">
        ...
        />
        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mt-3 mb-1">
          Category
        </label>
        <input
        ...
        <select v-model="draft.yield_configuration.mode" class="w-full rounded-lg border-slate-200 bg-slate-50">
      ...
      <div v-else-if="selected?.type === 'segment'">
        ...
        <input v-model="draft.label" class="w-full rounded-lg border-slate-200 bg-slate-50" />
        <div class="grid grid-cols-2 gap-3 mt-3">
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
            Start
            <input v-model="draft.time_start" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50" />
          </label>
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
            End
            <input v-model="draft.time_end" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50" />
          </label>
        </div>
        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mt-3 mb-1">
          Rate Card
        </label>
        <select v-model="draft.rate_card_id" class="w-full rounded-lg border-slate-200 bg-slate-50">
      ...
      <div v-else-if="selected?.type === 'overlay'">
        ...
        <div class="flex items-center gap-2 mt-3">
          <input id="hard-ticket" v-model="draft.is_hard_ticket" type="checkbox" class="rounded" />
      ...
      <div v-else-if="selected?.type === 'trigger'">
        ...
        <input v-model="draft.trigger_time" class="w-full rounded-lg border-slate-200 bg-slate-50" />
        <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mt-3 mb-1">
          Type
        </label>
        <select v-model="draft.type" class="w-full rounded-lg border-slate-200 bg-slate-50">
        ...
        <select v-model="draft.target_event" class="w-full rounded-lg border-slate-200 bg-slate-50">
          <option value="">None</option>
          <option v-for="event in overlayEvents" :key="event.id" :value="event.id">
            {{ event.label }}
          </option>
        </select>
      </div>
...
const selectedItem = computed(() => {
  ...
  if (props.selected.type === "rateCard") {
    return props.rateCards.find((item) => item.id === props.selected?.id) ?? null;
  }
  if (props.selected.type === "segment") {
    return props.flowSegments.find((item) => item.id === props.selected?.id) ?? null;
  }
  if (props.selected.type === "overlay") {
    return props.overlayEvents.find((item) => item.id === props.selected?.id) ?? null;
  }
  return props.logicTriggers.find((item) => item.id === props.selected?.id) ?? null;
});
