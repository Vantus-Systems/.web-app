<template>
  ...
  >
    <div
      class="flex items-center justify-between px-4 py-3 border-b border-slate-200 bg-white"
    >
      <div>
        <p
      ...
      <div class="flex gap-2">
        <button
          class="text-xs font-bold text-slate-500 border border-slate-200 px-2 py-1 rounded-lg"
          @click="$emit('prev')"
        >
          Prev
        </button>
    ...
    <div
      class="grid grid-cols-7 gap-px bg-slate-200 text-xs font-bold text-slate-500 uppercase tracking-widest"
    >
      <div
        v-for="day in weekDays"
        :key="day"
        class="bg-white px-2 py-2 text-center"
      >
        {{ day }}
      </div>
    </div>
      ...
      <div class="grid grid-cols-7 gap-px bg-slate-200 h-full">
        <div
          ...
          :class="{
            ...
            'border-primary-500 ring-2 ring-primary-100':
              selectedDates.includes(cell.date),
          }"
        ...
        >
          ...
          <div class="mt-2 flex-1 flex flex-col items-start justify-center">
            ...
            <div
              v-else-if="
                cell.assignment?.status === 'closed' && !cell.isFallback
              "
