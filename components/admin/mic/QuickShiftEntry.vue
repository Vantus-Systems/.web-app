    ...
    <form class="space-y-4" @submit.prevent="handleSubmit">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <div class="flex items-center mb-1">
            ...
            >
            <HelpTip
              text="The total cash in the drawer at the START of the shift. Auto-filled from previous shift."
            />
          </div>
          <div class="relative">
            <span
        ...
        <div>
          <div class="flex items-center mb-1">
            <label
              class="block text-xs font-bold text-emerald-600 uppercase tracking-wider"
              >Cash Count (Total)</label
            >
            <HelpTip
              text="Total cash in drawer BEFORE separating deposit. Drives the recursive calculation."
            />
          </div>
          <div class="relative">
            <span
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <div class="flex items-center mb-1">
            ...
            >
            <HelpTip
              text="The total cash remaining in the drawer AFTER deposit. Calculated from Cash Count."
            />
          </div>
          <div class="relative">
            <span
        ...
        <div>
          ...
          <div class="relative">
            ...
            <input
              ...
              @input="
                calculate('pulltabs');
                if (form.cash_count) calculate('cash_count');
              "
            />
          </div>
        </div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <div class="flex items-center mb-1">
            <label
              class="block text-xs font-bold text-slate-500 uppercase tracking-wider"
              >Bingo (Deposited)</label
            >
            <HelpTip
              text="The portion of the bank deposit that came from Bingo sales. Auto-calculated as Deposit - Pulltabs."
            />
          </div>
          <div class="relative">
            <span
          ...
          <p
            v-if="form.workflow_type === 'NEGATIVE_BINGO_BOX'"
        ...
        <div>
          <div class="flex items-center mb-1">
            <label
              class="block text-xs font-bold text-slate-500 uppercase tracking-wider"
              >Deposit to Bank</label
            >
            <HelpTip
              text="The actual amount of cash/checks physically taken to the bank."
            />
          </div>
          <div class="relative">
            <span
      ...
      <div class="bg-slate-50 rounded-lg p-3 border border-slate-200 space-y-2">
        <div
          ...
          class="text-xs text-amber-600 font-bold flex items-center gap-2"
        >
          <span>⚠️ Missing box counts</span>
          <HelpTip
            text="Please enter Starting and Ending Box amounts to calculate actual revenue."
          />
        </div>
        <template v-else>
          <div class="flex justify-between items-center text-xs">
            <div class="flex items-center">
              ...
              <HelpTip
                text="Ending Box - Starting Box. Positive means cash was added to the drawer."
              />
            </div>
            <span
              :class="
                derived.box_delta >= 0 ? 'text-emerald-600' : 'text-red-600'
              "
          ...
          <div
            class="flex justify-between items-center text-xs border-t border-slate-200 pt-2 mt-1"
          >
            <div class="flex items-center">
      ...
      <div
        v-if="showAdvanced"
        class="bg-slate-50 p-2 rounded border border-slate-200 text-xs mt-2"
      >
        <div class="flex items-center mb-1">
          <label class="block font-bold">Workflow Override</label>
          <HelpTip
            text="Manually set the accounting workflow. 'Auto' usually works best."
          />
        </div>
        <select
          v-model="form.workflow_type"
          class="w-full p-1 rounded border-slate-200 mb-2"
        >
        ...
        <input
          v-model="form.notes"
          class="w-full p-1 rounded border-slate-200"
