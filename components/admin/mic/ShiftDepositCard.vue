  ...
  >
    ...
    >
      <div>
        ...
        <p class="text-sm text-slate-500">
          Enter shift totals and deposit details
        </p>
      </div>

      <!-- View Toggle -->
      <div
        class="flex items-center bg-slate-100 rounded-lg p-1 self-start md:self-auto"
      >
        <button
          type="button"
    ...
    <div class="p-6">
      ...
      <div v-if="viewMode === 'workflow'">
        <ShiftWizard :initial-date="date" @submit="handleWorkflowSubmit" />
      </div>
      ...
      <div v-else class="space-y-8">
        <form class="space-y-8" @submit.prevent="handleFormSubmit">
            ...
            <div class="space-y-2">
              ...
              <div class="flex items-center gap-2">
                <button
                  ...
                  @click="
                    formData.headcount = Math.max(0, formData.headcount - 1)
                  "
                >
                  -
                </button>
          ...
          <div
            v-if="calculatedBingo < 0"
            class="animate-in fade-in slide-in-from-top-2"
          >
            <label class="block space-y-2">
              <span
                class="block text-xs font-bold text-rose-600 uppercase tracking-wider"
              >
          ...
          <div
            class="bg-slate-50 rounded-xl p-6 space-y-6 border border-slate-200"
          >
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div class="space-y-2">
                ...
                >
                  <div
                    class="flex justify-between items-center bg-slate-50 p-2 rounded mb-2"
                  >
                    <span class="text-xs font-bold text-slate-500">Total</span>
                    <span class="font-bold text-slate-900"
                      >${{ formatCurrency(currencyTrackerTotal) }}</span
                    >
                  </div>
              ...
              <div class="space-y-4">
                ...
                <label class="block space-y-2">
                  <span
                    class="block text-xs font-bold text-amber-600 uppercase tracking-wider flex items-center gap-1"
                  >
                    <svg
...
const totalDeposit = computed(() => {
  const currency = enableCurrencyTracker.value
    ? currencyTrackerTotal.value
    : formData.cash_currency;
...
const handleFormSubmit = async () => {
  ...
  const payload = {
    ...
    // Detailed breakdown
    cash_currency: enableCurrencyTracker.value
      ? currencyTrackerTotal.value
      : formData.cash_currency,
...
const loadExistingShift = async () => {
  try {
    ...
    const existing = Array.isArray(response)
      ? response.find((r: any) => r.date === props.date) ||
    ...
    if (existing) {
      ...
      formData.potential_problem_checks =
        existing.potential_problem_checks || "";
