  ...
  <form class="space-y-6" @submit.prevent="handleSubmit">
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
        Date
        <input v-model="draft.date" type="date" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50" />
      </label>
      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
        Shift
        <select v-model="draft.shift" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50" :disabled="noShifts">
          <option v-for="option in shifts" :key="option" :value="option">{{ option }}</option>
        </select>
      </label>
      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
        Workflow
        <select v-model="draft.workflow_type" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
        Pull Tabs Total
        <input v-model.number="draft.pulltabs_total" type="number" min="0" step="0.01" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50" />
      </label>
      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
        Deposit
        <input
      ...
      <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
        Players
        <input v-model.number="draft.players" type="number" min="0" step="1" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50" />
      </label>
    </div>

    <div v-if="draft.workflow_type === 'NEGATIVE_BINGO_BOX'" class="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-3">
      <div class="text-xs font-bold uppercase tracking-[0.3em] text-amber-700">
        Box Reconciliation
      </div>
      <label class="block text-xs font-bold text-amber-700 uppercase tracking-wider">
        Beginning Box
        <input v-model.number="draft.beginning_box" type="number" min="0" step="0.01" class="mt-1 w-full rounded-lg border-amber-200 bg-white" />
      </label>
      <label class="block text-xs font-bold text-amber-700 uppercase tracking-wider">
        Ending Box (≤ 4000)
        <input v-model.number="draft.ending_box" type="number" min="0" max="4000" step="0.01" class="mt-1 w-full rounded-lg border-amber-200 bg-white" />
      </label>
      <div class="flex items-center gap-2 text-xs text-amber-700">
        <input id="deposit-override" v-model="depositOverride" type="checkbox" class="rounded" />
        <label for="deposit-override">Override deposit lock</label>
      </div>
      <p class="text-xs text-amber-700">
        Deposit defaults to pull tabs. Bingo total is computed from the box change.
      </p>
    </div>

    <div v-if="draft.workflow_type === 'RECUPERATION_BOX_RETURN'" class="bg-indigo-50 border border-indigo-200 rounded-xl p-4 space-y-3">
      <div class="text-xs font-bold uppercase tracking-[0.3em] text-indigo-700">
        Recuperation Details
      </div>
      <label class="block text-xs font-bold text-indigo-700 uppercase tracking-wider">
        Beginning Box
        <input
      ...
      <label class="block text-xs font-bold text-indigo-700 uppercase tracking-wider">
        Ending Box (≤ 4000)
        <input v-model.number="draft.ending_box" type="number" min="0" max="4000" step="0.01" class="mt-1 w-full rounded-lg border-indigo-200 bg-white" />
      </label>
      <label class="block text-xs font-bold text-indigo-700 uppercase tracking-wider">
        Bingo Actual
        <input v-model.number="draft.bingo_actual" type="number" step="0.01" class="mt-1 w-full rounded-lg border-indigo-200 bg-white" />
      </label>
      <label class="block text-xs font-bold text-indigo-700 uppercase tracking-wider">
        Deposit Actual
        <input v-model.number="draft.deposit_actual" type="number" step="0.01" class="mt-1 w-full rounded-lg border-indigo-200 bg-white" />
      </label>
      <div class="flex items-center gap-2 text-xs text-indigo-700">
        <input id="beginning-override" v-model="beginningBoxOverride" type="checkbox" class="rounded" />
        <label for="beginning-override">Override beginning box</label>
      </div>
      <p class="text-xs text-indigo-700">
        Actuals account for recuperation of negative bingo. Deposit is computed from pull tabs + bingo actual.
      </p>
    </div>
    ...
    <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider">
      Notes
      <textarea v-model="draft.notes" rows="2" class="mt-1 w-full rounded-lg border-slate-200 bg-slate-50"></textarea>
    </label>

    <div class="flex justify-end gap-3">
      <button type="button" class="px-4 py-2 text-xs font-bold text-slate-500" @click="$emit('cancel')">
        Cancel
      </button>
      <button
        type="submit"
...
const bingoTotalDisplay = computed(() => {
  if (draft.value.workflow_type === "NEGATIVE_BINGO_BOX") {
    return ((draft.value.ending_box ?? 0) - (draft.value.beginning_box ?? 0)).toFixed(2);
  }
  if (draft.value.workflow_type === "RECUPERATION_BOX_RETURN") {
  ...
  return ((draft.value.deposit_total ?? 0) - (draft.value.pulltabs_total ?? 0)).toFixed(2);
});
  ...
  (workflow) => {
    ...
    if (workflow === "RECUPERATION_BOX_RETURN") {
      if (!beginningBoxOverride.value) {
        draft.value.beginning_box = props.prevEndingBox ?? draft.value.beginning_box;
      }
      if (draft.value.bingo_actual !== undefined) {
...
watch(
  () => props.prevEndingBox,
  (value) => {
    if (draft.value.workflow_type === "RECUPERATION_BOX_RETURN" && !beginningBoxOverride.value) {
...
watch(
  ...
  () => {
    if (draft.value.workflow_type === "NEGATIVE_BINGO_BOX" && !depositOverride.value) {
      draft.value.deposit_total = draft.value.pulltabs_total;
    }
    if (draft.value.workflow_type === "RECUPERATION_BOX_RETURN") {
      if (draft.value.bingo_actual !== undefined) {
