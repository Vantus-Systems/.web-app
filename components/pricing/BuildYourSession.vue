...
const estimatedCost = computed(() => {
  ...
  return (
    (machinePrice * (machineCount.value as number) - sundayDiscount) *
    playerCount.value
  );
});
...
const details = computed(() => {
  ...
  else if (machineCount.value !== "pay-as-you-go")
    lines.push(`${machineCount.value} Machine(s) per player`);
...
<template>
  <div
    class="bg-slate-900 rounded-2xl p-6 text-white shadow-xl border border-slate-700"
  >
    ...
    <div class="space-y-6">
      ...
      <div class="mt-8 pt-6 border-t border-slate-800">
        <div class="flex items-end justify-between mb-2">
          ...
          <span class="text-2xl font-black text-gold-400"
            >${{ estimatedCost }}</span
          >
        </div>
