...
const nextSession = computed(() => {
  ...
  // Ensure we match the schedule's day format ("Mon", "Tue"...)
...
<template>
  <div
    class="bg-slate-900 border-b border-gold-500/30 text-white relative z-40"
  >
    <div class="container mx-auto px-4">
      <div
        class="flex flex-col md:flex-row items-center justify-between py-2 md:h-12 gap-2 md:gap-4 text-sm"
      >
        <!-- Left: Status -->
        <div
        ...
        <div class="hidden lg:block text-slate-300 font-medium">
          <span class="text-emerald-400 font-bold">Today:</span>
          {{ specialText }}
        </div>

        <!-- Right: Actions -->
        <div
          class="flex items-center gap-4 w-full md:w-auto justify-center border-t border-slate-800 pt-2 md:pt-0 md:border-t-0"
        >
          <a
            :href="phoneLink"
          ...
          <button
            class="flex items-center gap-1.5 text-gold-400 hover:text-gold-300 transition-colors font-bold"
            @click="addToCalendar"
          >
