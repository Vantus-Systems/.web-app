  ...
  >
    <div class="space-y-6">
      <div class="flex flex-col gap-6">
        <div
          class="flex flex-col lg:flex-row gap-4 items-end lg:items-center justify-between"
        >
          ...
          <div class="flex flex-wrap gap-4 items-end">
            ...
            <div>
              ...
              >
              <select
                v-model="shiftFilters.userId"
                class="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary-500 outline-none min-w-[180px]"
              >
            ...
            <div>
              ...
              >
              <select
                v-model="shiftFilters.workflow"
                class="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-xs focus:ring-2 focus:ring-primary-500 outline-none min-w-[220px]"
              >
                <option value="">All</option>
                <option value="NORMAL">Normal</option>
                <option value="NEGATIVE_BINGO_BOX">Negative Bingo Box</option>
                <option value="RECUPERATION_BOX_RETURN">
                  Recuperation Box Return
                </option>
              </select>
            </div>
            <button
          ...
          <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
            <div class="flex flex-col">
              ...
              <span class="font-mono font-bold">{{
                formatCurrency(summary.depositBankSum)
              }}</span>
            </div>
            <div class="flex flex-col">
              <div class="flex items-center gap-1">
                ...
                >
                <HelpTip
                  text="Deposit to Bank + Box Change. If Deposit is $5,000 and Box Change is +$1,265, Actual Revenue is $6,265."
                />
              </div>
              <span
                class="font-mono font-bold"
                :class="
                  summary.actualRevenueSum >= 0
                    ? 'text-slate-900'
                    : 'text-red-600'
                "
                >{{ formatCurrency(summary.actualRevenueSum) }}</span
              >
            </div>
            <div class="flex flex-col">
              <span
              ...
              <span class="font-mono">{{
                formatCurrency(summary.pulltabsSum)
              }}</span>
            </div>
            <div class="flex flex-col">
              <span
              ...
              <span
                class="font-mono"
                :class="
                  summary.bingoActualSum >= 0
                    ? 'text-slate-900'
                    : 'text-red-600'
                "
                >{{ formatCurrency(summary.bingoActualSum) }}</span
              >
            </div>
            <div class="flex flex-col">
              <div class="flex items-center gap-1">
                <span
                  class="text-[10px] font-bold uppercase tracking-wider text-slate-500"
                  >Total Box Change</span
                >
                <HelpTip
                  text="If the box does not start and end the period at the same amount, totals may differ by the box change."
                />
              </div>
              <span
                class="font-mono"
                :class="
                  summary.boxChangeSum >= 0 ? 'text-slate-900' : 'text-red-600'
                "
                >{{ formatCurrency(summary.boxChangeSum) }}</span
              >
            </div>
          </div>
        </div>
...
const loadAllShifts = async () => {
  try {
    ...
    if (shiftFilters.value.workflow)
      params.workflow = shiftFilters.value.workflow;
...
const summary = computed(() => {
  ...
  const depositBankSum = rows.reduce(
    (acc, r) => acc + (r.deposit_total || 0),
    0,
  );
  const actualRevenueSum = rows.reduce(
    (acc, r) => acc + (r.actual_revenue ?? 0),
    0,
  );
  const pulltabsSum = rows.reduce((acc, r) => acc + (r.pulltabs_total || 0), 0);
  const bingoActualSum = rows.reduce(
    (acc, r) => acc + (r.bingo_actual ?? 0),
    0,
  );
...
const exportCsv = () => {
  ...
  const lines = allShifts.value.map((r) => [
    ...
    (
      r.bingo_deposited ?? (r.deposit_total ?? 0) - (r.pulltabs_total ?? 0)
    ).toString(),
