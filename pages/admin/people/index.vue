    ...
    <div class="grid grid-cols-1 lg:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] gap-6">
          ...
          <table class="min-w-full text-xs">
            ...
            <tbody>
              <tr v-for="user in users" :key="user.id" class="border-t border-slate-200">
                ...
                <td class="px-3 py-2">
                  <select v-model="user.role" class="rounded-lg border-slate-200 bg-slate-50 text-xs">
                ...
                <td class="px-3 py-2">
                  <input v-model="user.is_active" type="checkbox" class="rounded" />
                </td>
                <td class="px-3 py-2">
                  <button
      ...
      <div class="bg-white border border-slate-200 rounded-xl p-6 space-y-4">
        ...
        <form class="space-y-3" @submit.prevent="createUser">
          <input v-model="newUser.username" class="w-full rounded-lg border-slate-200 bg-slate-50 px-3 py-2 text-sm" placeholder="Username" />
          <input v-model="newUser.password" type="password" class="w-full rounded-lg border-slate-200 bg-slate-50 px-3 py-2 text-sm" placeholder="Password" />
          <div class="grid grid-cols-2 gap-2">
            <input v-model="newUser.firstName" class="w-full rounded-lg border-slate-200 bg-slate-50 px-3 py-2 text-sm" placeholder="First name" />
            <input v-model="newUser.lastName" class="w-full rounded-lg border-slate-200 bg-slate-50 px-3 py-2 text-sm" placeholder="Last name" />
          </div>
          <select v-model="newUser.role" class="w-full rounded-lg border-slate-200 bg-slate-50 px-3 py-2 text-sm">
            <option value="OWNER">Owner</option>
            <option value="MIC">MIC</option>
            <option value="CHARITY">Charity</option>
          </select>
          ...
          <button class="w-full bg-primary-900 text-white text-xs font-bold uppercase tracking-[0.3em] py-2 rounded-lg">
            Create
          </button>
        </form>
      </div>
