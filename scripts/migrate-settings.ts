...
async function main() {
  ...
  const existingScheduleActive = await prisma.scheduleVersion.findFirst({ where: { status: 'ACTIVE' } });
  ...
  const existingScheduleDraft = await prisma.scheduleVersion.findFirst({ where: { status: 'DRAFT' } });
  ...
  const existingPricingActive = await prisma.pricingVersion.findFirst({ where: { status: 'ACTIVE' } });
  ...
  const existingPricingDraft = await prisma.pricingVersion.findFirst({ where: { status: 'DRAFT' } });
